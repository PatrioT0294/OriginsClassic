...
diag_log format ["TEST DEBUG _dayz_onBack=%1",_dayz_onBack];
_dayz_onBackActive = player getVariable ["dayz_onBackActive",false];
diag_log format ["TEST DEBUG _dayz_onBackActive=%1",_dayz_onBackActive];
_primaryWeapon = primaryWeapon player;
diag_log format ["TEST DEBUG _primaryWeapon=%1",_primaryWeapon];
...
if (_dayz_onBack != "EMPTY" && !_dayz_onBackActive && !_inVehicle && !_onLadder) then {
	if (s_player_equip_carry < 0) then {
		...
		s_player_equip_carry = player addAction [_text2, "z\addons\dayz_code\compile\player_switchWeapon.sqf", "action", 0.5, false, true];
	};
} else {
...
if (_dayz_onBack == "EMPTY" && _primaryWeapon != "") then {
	if (s_player_equip_toback < 0) then {
		...
		s_player_equip_toback = player addAction [format["<t color=""#888888"">'%1' за спину</t>",_text], "z\addons\dayz_code\compile\player_addWeaponToBack.sqf", "action", 0.5, false, true];
